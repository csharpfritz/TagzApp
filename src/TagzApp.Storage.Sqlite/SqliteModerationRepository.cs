...
internal class SqliteModerationRepository : IModerationRepository
{
	...
	public SqliteModerationRepository(TagzAppContext context, INotifyNewMessages notifier, IMemoryCache cache)
	{
		_Context = context;
		_Notifier = notifier;
		_Cache = cache;
	...
	public async Task Moderate(string userId, string provider, string providerId, ModerationState state)
	{
		...
		var moderationAction = new SqliteModerationAction
		{
			Moderator = userId,
			Provider = provider,
	...
	public async Task BlockUser(string userId, string provider, string userName, DateTimeOffset expirationDate)
	{
		...
		var blockedUser = new SqliteBlockedUser
		{
